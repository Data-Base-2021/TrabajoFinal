...
select s.NSede,d.NDistrito,Count(i.CEmpleado) 'cantidad de usuarios',sum(pl.MPrecio)'Monto' from

Distrito d join Sede s on d.CDistrito=s.Distrito_CDistrito
...
create or alter procedure mesesmayorinscripcion as
begin
	Declare @promedio NUMERIC(10,2);

	select @promedio = AVG(total) from (
		select DATEPART(MONTH, DIncripcion) mes, count(1) total from Inscripcion
	...
	(  
		SELECT 
			ROW_NUMBER() OVER(ORDER BY value DESC) AS ID, value FROM STRING_SPLIT('ENERO,FEBRERO,MARZO,ABRIL,MAYO,JUNIO,JULIO,AGOSTO,SETIEMBRE,OCTUBRE,NOVIEMBRE,DICIEMBRE', ',')
	)   
	Select m.value, prom.total from 
		( select DATEPART(MONTH, DIncripcion) mes, count(1) total from Inscripcion
			group by DATEPART(MONTH, DIncripcion)
...
update producto set stock=4 where CProducto=115

select * from tb_productobajoenstock
 ...
 CREATE PROCEDURE EMPLEADOLIKEAPELLIDO 
...
SELECT 
 p.CPersona,p.NPersona,p.NApellidoPaterno,s.NSede FROM 
...
RETURN
(
select s.CSede,s.NSede,COUNT(i.CInscripcion) 'cantidad de inscripcion' from
Inscripcion i
join PlanSede ps on i.CPlan=ps.CPlan
join Sede s on s.CSede=ps.CSede  where s.CSede=@csede and( i.DIncripcion between @fecha_inicio and  @fecha_final)
group by s.CSede,s.NSede
)

SELECT * FROM masinscripcionesdeunasede(18610,'2016-01-01','2022-01-01')
